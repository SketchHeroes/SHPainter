package mx.core
{
   ...
   public class AdvancedLayoutFeatures extends Object implements IAssetLayoutFeatures
   {
      {
         reV.push(reVT);
         reV.push(reVR);
      ...
      private static function initTransformVectorFunction(param1:Matrix3D, param2:Vector3D) : Vector3D {
         ...
         {
            if(parseFloat(_loc4_[1]) > 0)
            {
               _loc3_=true;
            }
            else
            {
               if(parseFloat(_loc4_[2]) > 22)
               {
                  _loc3_=true;
               }
               else
               {
                  if(parseFloat(_loc4_[3]) >= 87)
                  {
                     _loc3_=true;
                  }
               }
